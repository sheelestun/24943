...
typedef struct Node_s {
    char *string;
    struct Node_s *next; //pointer for nxt node
} Node;

Node *head, *tail;

void init() {
    head = malloc(sizeof(Node)); // memory for fst node
...
void push(char *string) {
    ...
    char *copyPtr = calloc(len, sizeof(char)); //mem for str copy
    strcpy(copyPtr, string);

    tail->string = copyPtr;
    tail->next = calloc(1, sizeof(Node)); //epmty node
